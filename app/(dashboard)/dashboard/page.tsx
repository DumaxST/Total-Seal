...
import { getAllNotificationsByUser } from "@/app/lib/actions";
...
export default async function DashboardPage({ searchParams }: Props) {
  ...
  const { currentPage, totalPages, totalCount, } = await getPaginatedAlerts({ take: take, page: page });
  const alerts = await getAllNotificationsByUser() ?? []
  if (alerts?.length === 0) {
    // redirect('/');
  }
      ...
      <div className="col-span-8">
        <CardWrapper>
          ...
          <Suspense fallback={<SkeletonDashBoardTable />} >
            <TableDevices notifications={alerts} />
